...
- **Persistence is in-memory.** `engine/presistence.go` keeps a `map[string]*WorkflowExecution`. A process restart wipes every workflow.
...
- **Metrics are silently wrong.** `GetWorkflowMetrics` switches on `"Running"/"Completed"/"Failed"` but persistence stores `"RUNNING"/"COMPLETED"/"FAILED"`, so dashboard counters read zero.
...
- Long term history archival (S3, GCS). Out of scope for v2; revisit in v3.
...
- **Matching.** Owns task queues (workflow task queue, activity task queue, per shard). Backed by Redis Streams with consumer groups so polling workers get exclusive delivery and acks.
- **History.** Owns workflow state. Each shard is leased by exactly one history instance. Applies commands, appends events, persists in a single Postgres transaction, and enqueues resulting tasks to Matching.
