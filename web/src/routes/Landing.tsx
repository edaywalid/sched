...
client.RegisterActivity("SendEmail", func(ctx sdk.ActivityContext, addr any) (any, error) {
    log.Printf("sending to %v", addr)
    return nil, smtp.Send(addr.(string))
})`;
    ...
    <section className="border-b border-zinc-900">
      <div className="mx-auto max-w-3xl px-6 py-24">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <a
            href="#example"
            className="inline-flex items-center rounded-md border border-zinc-800 px-3.5 py-2 text-sm text-zinc-300 hover:border-zinc-700"
          >
            See the SDK
          </a>
        </div>
...
function ExampleSection() {
  return (
    <section id="example" className="border-b border-zinc-900">
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_2fr]">
          <div>
            ...
            <p className="mt-3 text-sm text-zinc-400">
              A workflow is a function. Calls to{" "}
              ...
              are recorded in durable history. If the worker dies, the
              engine re-dispatches the workflow against the same history
              and the SDK replays the function up to the next pending
              step.
            </p>
          </div>
...
function SiteFooter() {
  return (
    <footer>
      <div className="mx-auto flex max-w-5xl flex-col gap-3 px-6 py-10 text-xs text-zinc-500 sm:flex-row sm:items-center sm:justify-between">
        <div>
          sched. Apache 2.0. Built in Go, served with the same engine you see
          on this page.
        </div>
        <div className="flex items-center gap-5">
          <a href="https://github.com/edaywalid/sched" className="hover:text-zinc-300">
            GitHub
          </a>
          <Link to="/app" className="hover:text-zinc-300">
            Dashboard
          </Link>
        </div>
      </div>
